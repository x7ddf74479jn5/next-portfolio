    ...
    <article className="p-grid__list-item swiper-slide" key={index}>
      <Link href={sample.href}>
        <a className="u-text__line-none" target="_self">
          <h3>{sample.title}</h3>
          <div className="p-media__thumb">
            <Image width={960} height={540} style={{ objectFit: "cover" }} src={sample.img.src} alt={sample.img.alt} />
            {/* <Image priority layout="fill" style={{ objectFit: "cover" }} src={sample.img.src} alt={sample.img.alt} /> */}
            {/* <Image
              loading={"eager"}
              layout="fill"
